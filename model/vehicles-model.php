...
//function used to register a vehicle to db. 
...
//funcion used to update a vehicle
...
{
    $db = phpmotorsConnect();
    $sql = 'SELECT * FROM inventory INNER JOIN images ON inventory.invId=images.invId WHERE inventory.invId=:invId AND images.imgName NOT LIKE "$-tn%" ';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
    $stmt->execute();
...
{
    //Selects only primary vehicle thumbnail images using classification name. 
    $db = phpmotorsConnect();
...
//Selects information for all vehicles
